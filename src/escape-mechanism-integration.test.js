...
describe('Escape Mechanism Integration', () => {
  ...
  // Safe regex that splits on comma with optional spaces
  const splitArgsRegex = /\s*,\s*/;
  ...
  function mockApplyReplacers(doc) {
    return processWithEscapes(doc, content =>
      content.replace(replacerRegex, (match, type, args) => {
        ...
        return replacer ? replacer.transform(match, type, ...(args ? args.trim().split(splitArgsRegex) : [])) : match;
      })
    );
  }
  ...
  it('should process normal tags while preserving escaped ones', () => {
    ...
    expectNotContainsAny(result, ['__SPEC_UP_ESCAPED_TAG__']);
  });
